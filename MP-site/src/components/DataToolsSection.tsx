...
const tools = [{
  ...
  description: 'Explore differences in unstructured text between demographics in public reviews and comments.',
...
export function DataToolsSection() {
  return <section className="relative pt-12 pb-12 px-6 lg:px-12 bg-purple-900 border-b border-gray-800">
      {/* Black Oval Vignette */}
        ...
        <div className="flex flex-wrap gap-8 justify-center max-w-6xl mx-auto">
          {tools.map((tool, index) => <a key={index} href={`https://${tool.link}`} target="_blank" rel="noopener noreferrer" className="border border-purple-700 rounded-lg overflow-hidden hover:border-gauge-coral-2 transition-colors bg-purple-950 bg-opacity-50 text-center max-w-md w-full cursor-pointer group">
            ...
            aspectRatio: '16/9'
                ...
                <img src={tool.image} alt={tool.title} className="w-full h-full object-cover opacity-60 group-hover:opacity-80 transition-opacity" />
              </div>
              <div className="p-10">
                ...
                <span className="text-gauge-coral-2 font-sans text-base hover:underline">
                  {tool.link}
                </span>
              </div>
            </a>)}
