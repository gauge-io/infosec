...
export function PodcastBookingForm({ date, time, onClose, onSuccess }: PodcastBookingFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create calendar event - 20 minutes duration
      const eventDate = new Date(date);
      const [hours, minutes] = time.split(':').map(Number);
      eventDate.setHours(hours, minutes, 0, 0);
      
      const endDate = new Date(eventDate.getTime() + 20 * 60 * 1000); // 20 minutes duration

      // Build description with notes if provided
      // Note: Google Meet link will be automatically generated by Google Calendar API
      let description = `Podcast Intro Meeting with ${formData.name} (${formData.email})\n\nThis is a brief 20-minute introduction meeting to discuss potential podcast collaboration. A Google Meet link will be provided in the calendar invite.`;
      if (formData.notes.trim()) {
        description += `\n\nNotes from ${formData.name}:\n${formData.notes}`;
      }

      const eventSummary = `Podcast Intro Meeting w/ ${formData.name} and Nick Cawthon`;
      ...
      const result = await createCalendarEvent({
        ...
        meetingType: 'podcast' as const, // Flag for email template
      });
    ...
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="bg-gray-900 rounded-lg max-w-md w-full p-6 border border-gray-700">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-2xl font-serif font-semibold text-white">Book Podcast Intro</h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-6 p-4 bg-gray-800 rounded-lg">
          <div className="flex items-center gap-3 text-white">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">{time} (20 minutes)</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label htmlFor="notes" className="block text-sm font-sans font-medium text-gray-300 mb-2">
              Topic or question <span className="text-gray-500 text-xs">(optional)</span>
            </label>
            <div className="relative">
          ...
          {success && (
            <div className="p-4 bg-green-900/20 border border-green-500/50 rounded-lg">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-green-400 font-semibold">Podcast intro booked successfully!</p>
                  <p className="text-xs text-green-300 mt-1">A calendar invite with Google Meet link has been sent to your email.</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="flex gap-3 pt-4">
            ...
            >
              {loading ? 'Booking...' : 'Book Podcast Intro'}
            </button>
          </div>
        </form>
      </div>
